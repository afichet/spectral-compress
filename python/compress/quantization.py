...
def unbounded_get_error_progressive_quantization(wavelengths: np.array, ref: np.array, n_bits: int, err_fun=rrmse) -> np.array:
    phases = moments.wavelengths_to_phase(wavelengths)
    
    signal_to_moments = moments.get_basis_signal_to_moments(phases)
    moments_to_signal = np.linalg.inv(signal_to_moments)

    norm_moments, mins, maxs = moments.unbounded_forward(signal_to_moments, ref)

    err = np.zeros((ref.shape[2], ))

    for i in range(1, norm_moments.shape[2]):
        q_norm_moments = quantize_dequantize(norm_moments, i, n_bits)
        backward_signal = moments.unbounded_backward(moments_to_signal, q_norm_moments, mins, maxs)
...
def unbounded_generate_quantization_curve(wavelengths: np.array, ref: np.array, n_bits: int, err_fun=rrmse) -> np.array:
    phases = moments.wavelengths_to_phase(wavelengths)
...
def unbounded_err_for_curve(wavelengths: np.array, ref: np.array, curve: np.array, err_fun=rrmse):
    phases = moments.wavelengths_to_phase(wavelengths)
    ...
    backward_signal = moments.unbounded_backward(moments_to_signal, quantized_norm_moments, mins, maxs)
...
# =============================================================================
# Unbounded -> Bounded versions
# =============================================================================


def unbounded_to_bounded_get_error_progressive_quantization(wavelengths: np.array, ref: np.array, n_bits: int, err_fun=rrmse) -> np.array:
    phases = moments.wavelengths_to_phase(wavelengths)
    
    signal_to_moments = moments.get_basis_signal_to_moments(phases)
    moments_to_signal = np.linalg.inv(signal_to_moments)

    norm_moments, mins, maxs = moments.unbounded_to_bounded_forward(signal_to_moments, ref)

    err = np.zeros((ref.shape[2], ))

    for i in range(1, norm_moments.shape[2]):
        q_norm_moments = quantize_dequantize(norm_moments, i, n_bits)
        backward_signal = moments.unbounded_to_bounded_backward(moments_to_signal, q_norm_moments, mins, maxs)
...
def unbounded_to_bounded_generate_quantization_curve(wavelengths: np.array, ref: np.array, n_bits: int, err_fun=rrmse) -> np.array:
    phases = moments.wavelengths_to_phase(wavelengths)
...
def unbounded_to_bounded_err_for_curve(wavelengths: np.array, ref: np.array, curve: np.array, err_fun=rrmse):
    phases = moments.wavelengths_to_phase(wavelengths)
    ...
    backward_signal = moments.unbounded_to_bounded_backward(moments_to_signal, quantized_norm_moments, mins, maxs)
...
# =============================================================================
# Unbounded with max value
# =============================================================================


def bounded_w_upper_get_error_progressive_quantization(wavelengths: np.array, ref: np.array, n_bits: int, err_fun=rrmse) -> np.array:
    phases = moments.wavelengths_to_phase(wavelengths)
    
    signal_to_moments = moments.get_basis_signal_to_moments(phases)
    moments_to_signal = np.linalg.inv(signal_to_moments)

    norm_moments, relative_scale, mins, maxs, global_max = moments.bounded_w_upper_forward(signal_to_moments, ref)

    err = np.zeros((ref.shape[2], ))

    for i in range(1, norm_moments.shape[2]):
        q_norm_moments = quantize_dequantize(norm_moments, i, n_bits)
        backward_signal = moments.bounded_w_upper_backward(moments_to_signal, q_norm_moments, relative_scale, mins, maxs, global_max)
...
def bounded_w_upper_err_for_curve(wavelengths: np.array, ref: np.array, curve: np.array, err_fun=rrmse):
    phases = moments.wavelengths_to_phase(wavelengths)
    ...
    backward_signal = moments.bounded_w_upper_backward(moments_to_signal, quantized_norm_moments, relative_scale, mins, maxs, global_max)
